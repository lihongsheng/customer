...
class Register extends Event
{
    ...
    protected function linkTypeWork(array $data)
    {
        ...
        switch($msg['linkType']){
            case self::LINK_TYPE_GETWAY://来之getway服务器
                if($msg['eventType'] == self::EVENT_TYPE_LINK) {//link事件
                    echo '[getway] '.' LINK '.$msg['ip'].PHP_EOL;
                    $id = $data['key'];
